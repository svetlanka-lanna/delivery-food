const cartButton = document.querySelector("#cartButton"); /* при помощи правой сторны равенства помещаем в константу элемент с идентификатором (# - это идентификатор), с коротрым будем работать. То есть обращаемся к элементу, который имеет id в скобках */
...
close.addEventListener('click', function(event) {
    modal.classList.remove("is-open");                 /* получаем список классов  у элемента с классом modal и удаляем оттуда класс is-open, чтобы окно снова закрылось */
} );
...
function toggleModal() {
    modal.classList.toggle("is-open");       /* в этом случае класс не добавляется, а включается */     
}